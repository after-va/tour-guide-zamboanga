...
if (!$request || $request['tourist_ID'] != $_SESSION['user_id']) {
...
<div style="background: #f0f0f0; padding: 15px; border-radius: 5px; margin: 15px 0;">
    ...
    <p><strong>Email:</strong> <?php echo htmlspecialchars($request['guide_email']); ?></p>
    <p><strong>Phone:</strong> <?php echo htmlspecialchars($request['guide_phone']); ?></p>
</div>
...
<div class="message-thread">
    <?php if (count($messages) > 0): ?>
        <?php foreach ($messages as $msg): ?>
            <div class="message <?php echo $msg['role_ID'] == 3 ? 'message-tourist' : 'message-guide'; ?>">
...
<?php if ($request['request_status'] == 'pending' || $request['request_status'] == 'accepted'): ?>
    <form method="POST" action="my-requests.php" style="margin-top: 20px;">
        <input type="hidden" name="action" value="add_message">
        <input type="hidden" name="request_ID" value="<?php echo $request_ID; ?>">
        <div class="form-group">
            <label>Send Message to Guide:</label>
...
<?php if ($request['request_status'] == 'accepted'): ?>
    <div style="background: #d4edda; padding: 15px; border-radius: 5px; margin: 15px 0; text-align: center;">
        <strong>✓ This request has been accepted!</strong><br>
        <a href="payment-custom-package.php?request_id=<?php echo $request_ID; ?>" class="btn btn-success" style="margin-top: 10px;">Proceed to Payment</a>
    </div>
<?php endif; ?>
