...
trait TourPackagesTrait {
    ...
    public function getTourPackages($tour_ID) {
        ...
        if (!$db) {
            ...
            error_log("Database connection failed in getTourPackagesByTourID");
            return false;
        }

        $sql = "SELECT tp.* 
                FROM tour_packages tp
                INNER JOIN tour_tour_packages ttp ON tp.tour_package_ID = ttp.tour_package_ID
        ...
        try {
            ...
            $query->execute();
            $tourPackages = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function addTourPackages($package_name, $package_description){
        $db = $this->connect();
        $db->beginTransaction();
        try{
            $sql = "INSERT INTO tour_packages (tourpackage_name, tourpackage_description) 
                    VALUES (:package_name, :package_description)";
            $query = $db->prepare($sql);
